/*
 * The ATCTimer class is designed to manage and operate a hardware timer,
 * particularly in a real-time operating system using a message-passing mechanism to handle timer events.
 *
 ...
 *
 * The constructor (ATCTimer::ATCTimer) initializes the timer, setting up a message
 ...
 * indicating that the timer's set interval has elapsed.
 *
 * *****Time Measurement*****:
 ...
 * (via the setTimerSpecification(uint32_t sec, uint32_t nano) function)
 *
 */
 ...
 class ATCTimer {
	 ...
	 uint64_t cycles_per_sec; 			// System's cycles per second, for time calculation
 ...
 public:
	 ...
	 // Function to set the timer specifications (time intervals: seconds and nanoseconds)
